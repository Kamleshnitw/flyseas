            ...
            <tr>
                ...
                <td class="text-center">
                    @can('vendor-product-edit')
                        <a href="{{route('vendor.offer.edit',$data->id).'?key='.$search.'&page='.$list->currentPage()}}" class="btn btn-outline-primary btn-sm mr-1 mb-1">
                            <i class="fas fa-edit"></i>
                        </a>
                    @endcan
                    @can('vendor-product-delete')
                        <form action="{{route('vendor.offer.destroy',$data->id).'?key='.$search.'&page='.$list->currentPage()}}" method="POST">
                            @csrf
                            @method('delete')
                            <button type="submit" class='btn btn-outline-danger btn-sm' onclick="return confirm('Are you sure you want to delete this offer?');" style="width:32px;"><i class="fas fa-trash"></i></button>
                        </form>
                    @endcan
                </td>
            </tr>
